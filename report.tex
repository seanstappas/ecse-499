...
The Travis CI builds also ensure that are the dependencies are loaded properly, and also executes some standard code quality checks. These include \code{FindBugs} for finding bugs in the code, \code{checkstyle} for enforcing the Java Sun code style and \code{jacoco} for enforcing a certain test coverage.
